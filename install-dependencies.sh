...
echo
print_blue "================================="
print_blue " Hellsing - Dependencies Install "
print_blue "================================="
echo
echo
print_blue "This script will install Hellsing and all the required dependencies"
...
# Convert LIBRARIES string into an array
LIBRARIES_ARRAY=($LIBRARIES)

# Initialize an array to keep track of already installed libraries
installed_libraries=()

# Get a list of already installed packages (just names)
PIP3FREEZE=$(pip3 freeze | awk -F '==' '{print $1}' | tr '[:upper:]' '[:lower:]')

for lib in "${LIBRARIES_ARRAY[@]}"; do
    lib_lower=$(echo "$lib" | tr '[:upper:]' '[:lower:]') # Normalize case for comparison
    if echo "$PIP3FREEZE" | grep -i -q "^$lib_lower$"; then
        installed_libraries+=("$lib")
    else
        echo
        print_blue "[~] Install Python library ${lib} (py3)"
        if pip3 install "$lib"; then
            print_green "[+] Successfully installed ${lib}"
            ((newly_installed++))
            installed_libraries+=("$lib") # Add to installed after successful installation
        else
            print_red "[!] Failed to install ${lib}"
            ((failed_to_install++))
        fi
    fi
done
...
# Final report
total_libraries=${#LIBRARIES_ARRAY[@]}
total_installed=${#installed_libraries[@]}
echo
print_yellow "[~] Installation Summary:"
print_green "[+] $total_installed out of $total_libraries specified library(ies) are installed."
if [ $failed_to_install -gt 0 ]; then
    print_red "[!] Failed to install $failed_to_install library(ies)."
fi
...
if ! [ -x "$(command -v geckodriver)" ]; then
    ...
    if [ ${MACHINE_TYPE} == 'x86_64' ]; then
        ...
        tar -xvf geckodriver-v0.34.0-linux32.tar.gz
        rm -f geckodriver-v0.34.0-linux32.tar.gz
        mv geckodriver /usr/sbin
        if [ -e /usr/bin/geckodriver ]; then
            rm /usr/bin/geckodriver
        fi
...
print_blue "[~] Install Python3 libraries required by Hellsing (if missing)"
